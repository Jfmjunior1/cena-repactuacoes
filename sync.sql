-- ============================================================
-- Sincronizacao da planilha do Financeiro com o painel de repactuacoes
-- Novos ciclos vindos da planilha entram em revisao por seguranca.
-- ============================================================
...
-- Casos conhecidos que nunca devem ser promovidos automaticamente.
...
-- Todo ciclo novo criado automaticamente pela planilha nasce em revisao.
create or replace function public.aplicar_revisao_automatica_repactuacao()
returns trigger
language plpgsql
as $$
begin
  if new.fonte = 'planilha' and new.status = 'Previsto' then
    new.revisar := true;
  end if;
  return new;
end;
...
drop trigger if exists trg_repactuacoes_revisao_automatica on public.repactuacoes;
create trigger trg_repactuacoes_revisao_automatica
before insert on public.repactuacoes
for each row execute function public.aplicar_revisao_automatica_repactuacao();
...
begin
  ...
  for r in select * from jsonb_array_elements(p_dados) loop
    ...
    insert into public.contratos as t (
      id,sigla,cliente,unidade,garagem,m2,valor,rm2_controle,inicio,fim,
      ultima_reneg,proxima_reneg,historico,locador,administracao,situacao,
      ativo,visto_em,alterado_em)
    values (
      ...
      r->>'historico',r->>'locador',r->>'administracao',r->>'situacao',true,now(),now())
  ...
  update public.repactuacoes rp set
    ...
    prazo=case when c.fim is null or c.fim<current_date then 'Indet.'
      else lower(to_char(c.fim,'TMmon'))||'/'||to_char(c.fim,'YY') end,
    locador=c.locador,administracao=c.administracao,rm2_controle=c.rm2_controle,
    situacao_contrato=c.situacao
  from public.contratos c
  where rp.id like c.id||'|%'
    and rp.status is distinct from 'Concluído'
    and rp.valor_rep is null
    ...
    while v_n<v_max and (v_indet or v_data<=ct.fim) loop
      ...
      if not exists (select 1 from public.repactuacoes where id=v_rid) then
        insert into public.repactuacoes (
          id,ano,data,sigla,cliente,unidade,garagem,m2,valor,status,prazo,inicio,fim,
          ult_repactuacao,origem,locador,administracao,rm2_controle,situacao_contrato,
          sem_previsao,fonte,vista,revisar)
        values (
          ...
          ct.rm2_controle,ct.situacao,false,'planilha',ct.vista,true);
        v_cri:=v_cri+1;
        v_mud:=v_mud||jsonb_build_object('tipo','ciclo_novo','contrato',ct.id,'data',v_data,'revisar',true);
      end if;
  ...
  -- Contratos removidos da planilha e casos especiais permanecem para revisao.
  update public.repactuacoes rp set revisar=true
  from public.contratos c
  where rp.id like c.id||'|%'
    and rp.status='Previsto'
    and rp.fonte='planilha'
    and (not c.ativo or public.contrato_exige_revisao(c.sigla,c.cliente,c.unidade,c.valor,c.situacao))
    and rp.revisar=false;

  insert into public.sincronizacoes (
    origem,recebidos,novos,alterados,ausentes,ciclos_criados,ciclos_ajustados,mudancas)
  values (
    'power-automate',jsonb_array_length(p_dados),v_novos,v_alt,v_aus,v_cri,v_ajs,v_mud)
  returning id into v_sinc;

  return jsonb_build_object(
    'ok',true,'sincronizacao',v_sinc,'recebidos',jsonb_array_length(p_dados),
    'novos',v_novos,'alterados',v_alt,'ausentes',v_aus,
    'ciclos_criados',v_cri,'ciclos_ajustados',v_ajs);
end $$;
